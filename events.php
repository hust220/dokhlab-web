...
foreach ($etag as $k=>$v):
	...
	array_push($days[$etag[ 'edate']], array($etag[ 'edescription'],$etime,implode("&", $kvp), $etag[ 'edescription']));
	endforeach;
	//print_r($days);
...
$pn = array('&laquo;'=>"/dokhlab/events.php?year=$prev_year&month=$prev_month",'&raquo;'=>"/dokhlab/events.php?year=$next_year&month=$next_month");
...
for($day=1,$days_in_month=gmdate('t',$first_of_month); $day<=$days_in_month; $day++,$weekday++){ 
	...
	if(isset($days["$month-$day-$year"]) and is_array($days["$month-$day-$year"])){ 
		...
		foreach ($days[ "$month-$day-$year"] as $dayarr):
			@list($link, $desc, $edetails, $edesc) = $dayarr; 
		if($edesc == "Group Meeting") {
			$calendar .= getGM($month, $day, $year);
		}
		if($edesc == "Journal Club") {
			$calendar .= getJC($month, $day, $year);
		}
		endforeach;
		$calendar .= '</div></td>';
	} 
...
//	echo generate_calendar($year, $month, $days, $day_name_length, $first_day, $pn);
echo $calendar."</tr>\n</table>\n";
echo '<table width=700 align=center><tr><td align=right class="lnText"><span class="genBorderGray">Manage Events</span></td></tr></table>';
echo '</div>';
?>
