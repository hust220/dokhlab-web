		...
		if(isset($days["$month-$day-$year"]) and is_array($days["$month-$day-$year"])){ 
			...
			foreach ($days[ "$month-$day-$year"] as $dayarr):
				@list($link, $desc, $getstr) = $dayarr; 
					$calendar .= '<a href="showevent.php?'.$getstr.'" class="info" rel="lyteframe" title="Event Description" rev="width: 500px; height: 400px; scrolling: no;">'.$desc.'<span>'.$link.'</span></a><br>'; 
			endforeach;
			$calendar .= '</div></td>';
		} 
			else {
